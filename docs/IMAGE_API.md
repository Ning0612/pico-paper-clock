...
名稱由 1–48 個 ASCII 字母、數字、`_`、`-` 加上 `.bin` 組成。新工具上傳的 payload 一律是帶 PPC1 header 的 bitmap；PPC1 header 保存 bit order，裝置以 bounded streaming decoder 還原。API 仍接受既有固定長度 raw `MONO_HLSB`，並以 `.bin.hlsb` marker 維持舊資產相容性；無 marker 的既有 raw 圖片仍按舊版 MSB-left 讀取。
