...
class DashboardUiTests(unittest.TestCase):
    ...
    def test_generated_asset_keeps_statement_boundary_after_minification(self):
        ...
        self.assertIn('id="heatmapPrev"', self.asset)
        self.assertIn('id="heatmapNext"', self.asset)
        self.assertIn('id="heatmapPeriod"', self.asset)
    ...
    def test_dashboard_heatmap_supports_recent_default_and_calendar_year_arrows(self):
        for token in (
            'id="heatmapPrev"',
            'id="heatmapNext"',
            'id="heatmapPeriod"',
            ...
            "heatmapYear===null",
            "period.textContent=heatmapYear===null?'最近 365 天':heatmapYear+' 年'",
            "selectHeatmapYear(null)",
        ...
        self.assertIn('aria-label="較早年份"', self.source)
        self.assertIn('aria-label="較新年份"', self.source)
